            ...
            if result.data:
                ...
                if daily_task.data:
                    ...
                    if last.data and last.data[0].get("completed_at"):
                        ...
                        can_claim = now - last_time > timedelta(hours=24)
                    if can_claim:
            ...
            if t.get("is_daily"):
                ...
                if last.data and last.data[0].get("completed_at"):
                    ...
                    if diff < timedelta(hours=24):
                        ...
                        t["done"] = True
                        t["cooldown_seconds"] = int(remaining.total_seconds())
                    else:
                        t["done"] = False
...
def admin_find_user():
    ...
    gifts = supabase.table("user_gifts").select("*").eq("user_id", u["id"]).execute().data
